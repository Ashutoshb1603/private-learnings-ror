...
describe AccountBlock::SocialAccount, :type => :model do
  before(:all) { @account = build(:account) }
  subject { @account }
  # let (:subject) { build :account_block/social_account }
  context "validation" do
    ...
    it "should validate_presence_of uniqu_auth_id" do
      expect(subject).to be_valid
    end
  ...
  context "set_active" do
    it "changes the status to active" do
      subject.set_active
      expect(subject.activated).to eql(true)
    end
  end
