...
ActiveAdmin.register_page "UK" do
  require "uri"
  require "net/http"
  ...
  content do
    ...
    if uk_key
      ...
      uk_products = crypt.decrypt_and_verify(uk_key)
      begin
    ...
    when 1
      products = JSON.parse(uk_shopify("/admin/api/2021-10/products.json?status=draft&limit=250"))['products']
    when 2
      products = JSON.parse(uk_shopify("/admin/api/2021-10/products.json?status=archived&limit=250"))['products']
    when 0
      products = products
    end
    ...
    if params[:search].present?
      response = JSON.parse(uk_shopify("/admin/api/2021-10/products.json?limit=250"))
      products = response['products']
      while response['next'] != ""
        next_page = response['next']
        response = JSON.parse(uk_shopify("/admin/api/2021-10/products.json?page_info=#{next_page}"))
        products += response['products']
      end
    ...
    products.each do |product|
      product['price'] = product['variants'][0]['price'] + " " + "GBP"
    end
    last_update = BxBlockCatalogue::ProductKey.where(location: "Uk").last.last_refreshed
      ...
      div class: 'sidebar_section panel', id: 'filters_sidebar_section' do
        ...
        div class: 'mb-3 d-flex align-items-end filterBlock' do
          div class: 'status_div' do
            label "Status"
           select id: 'filter_by_status', collection: [["Active", 0], ["Draft", 1], ["Archived", 2]], propmt: 'Select status'
          end
          ...
          div do
            link_to "Filter", admin_uk_path, class: 'link', id: "filter_products", style: "padding:20% 12px"
          end
          div do
            link_to "Refresh products", "", class: 'link', id: "refresh_button", value: "Uk", onclick: "refresh()", style: "padding:4% 12px;"
          end
          div do
