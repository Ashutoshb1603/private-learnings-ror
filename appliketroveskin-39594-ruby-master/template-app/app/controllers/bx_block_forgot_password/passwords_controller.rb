module BxBlockForgotPassword
  class PasswordsController < ApplicationController
    # def create
    #   if (request.headers[:token].present? || create_params[:token].present?) && create_params[:new_password].present?
    #     # Try to decode token with OTP information
    #     user_token = request.headers[:token] || create_params[:token]
    #     begin
    ...
    #           account.update(activated: true) unless account.activated
