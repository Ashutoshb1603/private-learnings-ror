  ...
  class UploadMediaController < ApplicationController
      ...
      if params[:media].present? && params[:media][:meta].present?
        params[:media][:meta].each do |media_meta|
          ...
          if upload.save
            ...
            return render json: {errors: upload.errors}, status: 400
          end
        end
        render json: {data: urls}
      else
        render json: {errors: "media or meta missing"}
      end
    end
    ...
    def bulk_upload
      if params[:media].present?
        ...
        render json: {errors: "media missing"}
      end
    end
    ...
    def index
      ...
      if seller_account.present?
        ...
        render json: {message: "No data is found with this account"}
      end
    end
